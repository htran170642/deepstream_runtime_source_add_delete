...
std::vector<int> g_running_source_id_list(MAX_NUM_SOURCES);
...
{
// create_uridecode_bin (guint index, guint new_source_id, gchar * filename)
// {
  GstElement *bin = NULL;
  gchar bin_name[16] = { };
  ...
  // g_running_source_id_list.append(new_source_id);
...
add_sources (gpointer data)
{
  ...
  if (g_num_sources == MAX_NUM_SOURCES) {
     ...
     */
    g_timeout_add_seconds (15, delete_sources, (gpointer) g_source_bin_list);
    return FALSE;
  }
...
static void getCurrentCameraState(std::vector<Camera> &cameras) {
  if (!crud.connect()) {
      g_print("=>>>>>>>>>>>>>>>>>>>>>> Could not connect to database\n");
      return;
  }
  cameras = crud.getActiveCameras();
  g_print("=>>>>>>>>>>>>>>>>>>>>>> active cameras size = %d\n", cameras.size());
}

static gboolean check_new_sources_watchdog (gpointer data) {
  g_print ("=>>>>>>>>>>>>>>>>>>>>>> check_new_sources_watchdog\n");
  std::vector<Camera> cameras;
  getCurrentCameraState(cameras);
  if (cameras.size()) {
...
main (int argc, char *argv[])
{
  ...
  if ((argc != 4)) {
    g_printerr ("Usage: %s <run forever> <sink> <sync>\n", argv[0]);
  ...
  if (!strcmp(argv[2],"filesink")){
    display = FALSE;
  }
  else if  (!strcmp(argv[2],"nveglglessink") || !strcmp(argv[2],"nv3dsink") ){
    display = TRUE;
  }
  else {
  ...
  std::vector<Camera> active_cameras = crud.getActiveCameras();
  num_sources = active_cameras.size();
  g_num_sources = num_sources;
  for (i = 0; i < num_sources; i++) {
    // GstElement *source_bin = create_uridecode_bin (i, active_cameras[i].source_id, active_cameras[i].rtsp_url.c_str());
    GstElement *source_bin = create_uridecode_bin (i, active_cameras[i].rtsp_url.c_str());
    if (!source_bin) {
      g_printerr ("Failed to create source bin. Exiting.\n");
      return -1;
    }
  ...
  g_timeout_add_seconds (5, check_new_sources_watchdog, (gpointer) g_source_bin_list);
