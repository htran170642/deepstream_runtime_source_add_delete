...
const string host = "192.168.1.6:3306";
const string user = "root";
const string password = "admin";
const string database = "ds_streams";
static MySQLCRUD crud(host, user, password, database);
...
std::map<int, int> camera_maps; // index - source_id  
...
static gboolean add_sources (Camera &cam);
static gboolean check_new_sources_watchdog (gpointer data);

template <typename K, typename V>
K findKeyByValue(const std::map<K, V>& myMap, const V& targetValue) {
    for (const auto& pair : myMap) {
        if (pair.second == targetValue) {
            return pair.first;  // Return the key when the value matches
        }
    }
    // Return a default key or throw an exception if the value is not found
    // You can decide what to do in this case. My case will return -1
    return -1;
}
...
typedef struct
{
    ...
    gdouble fps[MAX_NUM_SOURCES];
    gdouble fps_avg[MAX_NUM_SOURCES];
...
perf_cb (gpointer context, NvDsAppPerfStruct * str)
{
  ...
  //     g_print ("cam%d: %.2f (%.2f)\t", i, thCtx->fps[i], thCtx->fps_avg[i]);
  // }

  for (const auto& pair : camera_maps) {
    int index = pair.first;
    int source_id = pair.second;
    g_print ("cam%d: %.2f (%.2f)\t", source_id, thCtx->fps[index], thCtx->fps_avg[index]);
  }
  g_print ("\n");
...
delete_sources (Camera &cam)
{
  ...
  source_id = findKeyByValue(camera_maps, cam.source_id);
  g_source_enabled[source_id] = FALSE;
...
static gboolean
add_sources (Camera &cam)
{
  if (g_num_sources == MAX_NUM_SOURCES) {
    g_print ("Reaching Max source = %d \n", MAX_NUM_SOURCES);
    return FALSE;
  }
  g_num_sources++;
  ...
  camera_maps.insert(std::make_pair(source_id, cam.source_id));

  g_print ("Calling Start %d \n", source_id);
  source_bin = create_uridecode_bin (source_id, (char *)(cam.rtsp_url).c_str());
  if (!source_bin) {
...
static void getCurrentCameraState(std::vector<Camera> &active_cameras, std::vector<Camera> &inactive_cameras) {
  if (!crud.connect()) {
      g_print("Could not connect to database\n");
      return;
  }
  active_cameras = crud.getActiveCameras();
  inactive_cameras = crud.getInactiveCameras();
  // g_print("active cameras size = %ld\n", active_cameras.size());
  // g_print("inactive cameras size = %ld\n", inactive_cameras.size());
}

static gboolean check_new_sources_watchdog (gpointer data) {
  g_print ("\n===> check_new_sources_watchdog\n");
  std::vector<Camera> active_cameras;
  std::vector<Camera> inactive_cameras;
  getCurrentCameraState(active_cameras, inactive_cameras);
  if (active_cameras.size()) {
      for(auto &cam: active_cameras) {
          int is_added = add_sources(cam);
          if (is_added) {
              if (!crud.updateCamera(cam.source_id, cam.rtsp_url, true, cam.use_case, cam.camera_location, true)) {
                  g_print ("Failed to update a camera record - source_id = %d\n", cam.source_id);
              } else {
                  g_print ("update camera %d successfully\n", cam.source_id);
              }
          }
      }
  }

  if (inactive_cameras.size()) {
      for(auto &cam: inactive_cameras) {
          // check a specific camera if it is off and if it is, remove it from running cameras list
          int found_source_id = findKeyByValue(camera_maps, cam.source_id);
          if (found_source_id > 0) {
              g_print("found_source_id = %d with source_id = %d is off\n", found_source_id, cam.source_id);
              if (delete_sources(cam)) {
                  camera_maps.erase(found_source_id);
              }
          } else {
              g_print("source_id = %d is not in running camera list\n", cam.source_id);
          }
      }
  }

  // Iterate through the map
  // g_print ("camera maps size = %d\n", camera_maps.size());
  // for (const auto& cam : camera_maps) {
...
main (int argc, char *argv[])
{
  ...
  for (i = 0; i < /*num_sources */ 1; i++) {
    ...
    gst_bin_add (GST_BIN (pipeline), source_bin);
    camera_maps.insert(std::make_pair(0, 0)); // init first camera
  }
  ...
  if (!pgie || !sgie1 || !tiler || !nvvideoconvert || !nvosd
      || !sink || !tracker) {
    g_printerr ("One element could not be created. Exiting.\n");
    return -1;
  }
  ...
  gst_bin_add_many (GST_BIN (pipeline), pgie, tracker, sgie1,
  ...
  if (!gst_element_link_many (streammux, pgie, tracker, sgie1, queue,
          tiler, nvvideoconvert, nvosd, sink, NULL)) {
    g_printerr ("Elements could not be linked. Exiting.\n");
    return -1;
  }
  ...
  GstPad *tiler_pad =  gst_element_get_static_pad (tiler, "sink");
  if (!tiler_pad)
    ...
    enable_perf_measurement (str, tiler_pad, MAX_NUM_SOURCES, 5, 0, perf_cb);
  }
  ...
  g_timeout_add_seconds (5, check_new_sources_watchdog, NULL);
